...
### In Oracle SOURCE
```sql
SELECT LOWER(grantee) AS grantee,
       LOWER(table_name) AS table_name,
       LOWER(CASE privilege WHEN 'READ' THEN 'SELECT' ELSE privilege END) AS privilege
FROM user_tab_privs
ORDER BY 1, 2, 3;
```
...
ora = pd.read_csv('oracle_privs.csv')
pg = pd.read_csv('postgres_privs.csv')
